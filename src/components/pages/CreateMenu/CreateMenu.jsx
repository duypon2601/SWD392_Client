...
import { Table, Button, InputNumber, message, Card, Typography, Space } from "antd";
...
const CreateMenu = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "action",
      render: (_, record) => (
        <Button type="primary" onClick={() => addToMenu(record.food_id)} style={{ backgroundColor: "#52c41a", borderColor: "#52c41a" }}>
          Add to Menu
        </Button>
      ),
    },
  ...
  return (
    <Card style={{ 
      ...
      <Title level={2} style={{ textAlign: "center", marginBottom: "20px", color: "#333" }}>🍽️ Create Menu</Title>
      <Table 
        dataSource={foodList || []} 
        columns={columns} 
        rowKey="food_id"
